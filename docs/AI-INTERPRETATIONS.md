...
## 1. 사전 작성 텍스트 + 실시간 LLM (2단계 구조)

5/18 피드백 #5("AI 해석·분석 기능")는 **두 단계**로 구현됐다.

**1단계 — 사전 작성 텍스트(기본/폴백)**:
- 보고서·인터뷰·현장조사 자료를 기반으로 시군별 해석을 사람이 사전 작성(`ai-interpretations.json`).
- "💡 AI 해석" 카드로 노출, **"사전 작성" 배지**로 표시.

**2단계 — 실시간 LLM (0531 추가, MindLogic UOS Gateway)**:
- `js/config.js`의 `LLM_PROXY_URL`이 설정되면 카드에 **"🤖 실시간 AI 해석" 버튼**이 붙는다.
- 클릭 시 Cloudflare Worker 프록시(`worker/`)를 통해 LLM이 지표·비전 적합도·발화 트리거를 근거로 **시군·읍면 맞춤 해석**을 생성 → 카드 본문 교체 + **"실시간 LLM" 배지**.
- 결과는 sessionStorage 캐시(같은 지역 즉시), "↻ 재생성"으로 갱신.
- **`LLM_PROXY_URL`이 비면 자동으로 1단계(사전 작성)로 폴백** → 배포 환경에 따라 무중단.
- ⚠️ 비밀키는 정적 사이트에 두지 않는다. 키는 Worker secret(`wrangler secret`)·로컬 `worker/.dev.vars`(gitignore)에만. 배포·구조는 [`worker/README.md`](../worker/README.md) 참조.
