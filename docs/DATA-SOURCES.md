...
|-----|----------|--------------|----------|
| `main_stats` | `stats/population.json` | tot_ppltn, avg_age, ppltn_dnsty, **aging_idx**, tot_house, corp_cnt, nongga_cnt | L2 노령화지수 |
...
### 6-3. 현재 상태 (자격증명 활성화 대기)

- 자격증명 (Service ID·Secret Key) 발급 완료 (2026-05-17)
- **인증 API (`addr/stage.json`) 작동 확인** ✅ — 17개 시도 정상 반환
- **통계 API 응답**:
  - `population.json`, `house.json`, `searchpopulation.json` → `errCd=-100 검색결과가 존재하지 않습니다`
  - `company.json` → `errCd=0 Success`이지만 `result=[]` 빈 응답
  - `farmhousehold.json`, `forestryhousehold.json` → `errCd=-200 년도 정보 확인 필요` (2015·2020·2021·2022 모두 실패)
- **추정 원인**: 신청 당일이라 통계 데이터 접근 권한이 아직 활성화 안 됨 (보통 1-3일 소요)
- **활성화 후**: `python fetch_sgis.py` 1회 실행으로 위 9개 endpoint 모두 수집 → L2·W2·W8·농가 등 즉시 채워짐
...
| `L2_aging_index` | 65세↑/0-14세×100 (SGIS 직접) | `aging_idx` (SGIS) | — |
