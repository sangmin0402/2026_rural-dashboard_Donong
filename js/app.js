...
function renderIndicatorVerdict(cityId, key) {
  ...
  // 0602: 근거·출처 칩 행 (해석의 출처·객관적 비교 기준 표면화)
  const chips = [];
  const m = (typeof refMeta === 'function') ? refMeta(key) : null;
  if (m && m.primary_source) chips.push(makeFactChip('source', m.primary_source, { tooltip: '1차 데이터 출처' }));
  if (m && m.secondary_source && m.secondary_source !== '—') chips.push(makeFactChip('source', m.secondary_source, { icon: '📎', tooltip: '2차 출처' }));
  if (fact) {
    const rTier = tier === 'high' ? 'high' : tier === 'low' ? 'low' : 'mid';
    chips.push(makeFactChip('rank', `${fact.scopeLabel} ${fact.rank}/${fact.total}위`, { tier: rTier, tooltip: `평균 ${fact.avg}${fact.unit} · 최고 ${fact.max}(${fact.maxRegion}) · 최저 ${fact.min}(${fact.minRegion})` }));
    chips.push(makeFactChip('rank', `경기평균 ${fact.avg}${fact.unit}`, { icon: '📐', tooltip: '경기 15개 시군 평균' }));
  }
  // 객관적 비교 기준 (전국농촌·유사지자체 — placeholder면 '데이터 대기')
  const m2 = (typeof refMeta === 'function') ? refMeta(key) : null;
  chips.push(makeBenchmarkChips(key, m2 && m2.unit));
  const chipRow = chips.length ? `<div class="fact-chip-row ivd-chips">${chips.join('')}</div>` : '';
  return `<div class="ind-verdict" aria-label="${escapeHtml(cityName)} ${key} 해석">${rows.join('')}${chipRow}</div>`;
}

/**
 * 0602: AI 판단 근거 패널 — LLM이 받은 facts(JS가 계산한 값·순위·평균)를 사용자에게 노출.
 * "AI가 무슨 근거로 말하는지"를 투명하게 보여 객관성·신뢰 확보 (#4).
 * @param {'eup'|'sigun'} scope
 * @param {string} regionId 읍면명 또는 시군 id
 */
function buildLlmEvidencePanel(scope, regionId) {
  const facts = [];
  try {
    if (scope === 'eup') {
      ['L1', 'L2', 'L4', 'L6', 'W5', 'W6', 'W7', 'W9', 'R3', 'R6', 'R7'].forEach(k => {
        const f = (typeof eupIndicatorFact === 'function') ? eupIndicatorFact(regionId, k) : null;
        if (f) facts.push(f);
      });
    } else {
      Object.keys(INDICATORS).forEach(k => {
        const f = (typeof sigunIndicatorFact === 'function') ? sigunIndicatorFact(regionId, k) : null;
        if (f) facts.push(f);
      });
    }
  } catch (_) {}
  if (!facts.length) return '';
  const tr = facts.map(f => {
    const tier = (f.tierLabel.includes('상위') || f.tierLabel.includes('강점')) ? 'high'
      : (f.tierLabel.includes('하위') || f.tierLabel.includes('보강')) ? 'low' : 'mid';
    const dir = f.higherBetter === false ? ' ↓' : '';
    return `<tr><td>${escHtml(f.name)}${dir}</td>`
      + `<td class="le-val">${f.value}${escHtml(f.unit)}</td>`
      + `<td>${makeFactChip('rank', `${f.rank}/${f.total}위`, { tier })}</td>`
      + `<td class="le-avg">평균 ${f.avg}</td>`
      + `<td>${makeFactChip('source', f.source)}</td></tr>`;
  }).join('');
  return `<details class="llm-evidence"><summary>🔎 판단 근거 — 대시보드가 계산한 사실 (${facts.length}개 지표)</summary>`
    + `<p class="le-note">아래 값·순위·평균은 LLM이 아니라 대시보드가 직접 계산했습니다. AI는 이 사실만 근거로 서술합니다(숫자 환각 차단).</p>`
    + `<div class="le-wrap"><table class="le-tbl"><thead><tr><th>지표</th><th>값</th><th>순위</th><th>비교</th><th>출처</th></tr></thead><tbody>${tr}</tbody></table></div></details>`;
}
...
function recordLlmUsage(data) {
  if (!data) return;
  const u = data._usage || data.usage;
  if (!u) { llmUsage.calls++; if (data._model) llmUsage.model = data._model; return; }
  llmUsage.calls++;
  llmUsage.in += Number(u.prompt_tokens || u.input_tokens || u.in || 0);
  llmUsage.out += Number(u.completion_tokens || u.output_tokens || u.out || 0);
  if (data._model) llmUsage.model = data._model;
}
...
/**
 * 0602: 랜딩 "남양주 2035 비전" 섹션 라이브 채움 (vision2035 JSON 기반).
 * 데이터 없으면 정적 폴백 유지.
 */
function renderLandingVision() {
  if (!vision2035 || !vision2035.areas) return;
  const barsEl = document.getElementById('lv-area-bars');
  if (barsEl) {
    const order = [['samlter', 'samlter'], ['ilter', 'ilter'], ['shimter', 'shimter']];
    const html = order.map(([k, cls]) => {
      const a = vision2035.areas[k];
      if (!a) return '';
      const low = a.score < 50 ? ' lv-bar-val--low' : '';
      return `<div class="lv-bar"><span class="lv-bar-lab">${escHtml(a.label)}</span>`
        + `<span class="lv-bar-track"><span class="lv-bar-fill lv-bar-fill--${cls}" style="width:${Math.max(0, Math.min(100, a.score))}%"></span></span>`
        + `<span class="lv-bar-val${low}">${a.score}<small>점</small></span></div>`;
    }).join('');
    if (html) barsEl.innerHTML = html;
  }
...
function updateDetailPanel(cityId) {
  ...
  // 0602: 시군 종합 진단 요약 (강점/부족/우선과제/다음할일) — 점수카드 직후
  renderSigunDiagSummary(cityId);
  ...
  // 2035 도시기본계획 비전 달성률 카드 (0531 박세희) — 남양주만
  renderVision2035Card(cityId);
...
function applyLlmToSigunCard(card, cityId, d) {
  ...
  // 0602: AI 판단 근거 패널 (JS가 계산한 facts 노출)
  let ev = card.querySelector('.llm-evidence-host');
  if (!ev) { ev = document.createElement('div'); ev.className = 'llm-evidence-host'; pol.insertAdjacentElement('afterend', ev); }
  ev.innerHTML = buildLlmEvidencePanel('sigun', cityId);
...
/**
 * 2035 도시기본계획 비전 달성률 카드 (0531 박세희) — 남양주만.
 * 시 전체 절대 목표(2035 도시기본계획) 대비 현황 달성률(%)을 막대로 표시.
 */
function renderVision2035Card(cityId) {
  const existing = document.getElementById('vision-2035-card');
  if (existing) existing.remove();
  if (cityId !== 'namyangju' || !vision2035) return;

  const rateColor = (r) => r < 50 ? '#B5182B' : r < 80 ? '#D97706' : '#2D6A4F';
  const bars = (vision2035.numeric || []).map(n => `
    <div class="v35-row">
      <div class="v35-row-head">
        <span class="v35-name">${escapeHtml(n.name)} <span class="v35-link">${escapeHtml(n.link || '')}</span></span>
        <span class="v35-rate" style="color:${rateColor(n.rate)}">${n.rate}%</span>
      </div>
      <div class="v35-bar"><div class="v35-bar-fill" style="width:${Math.min(100, n.rate)}%;background:${rateColor(n.rate)}"></div></div>
      <div class="v35-vals">현황 ${n.current}${escapeHtml(n.unit)} → 2035 목표 ${n.target}${escapeHtml(n.unit)}</div>
    </div>`).join('');

  const areas = vision2035.areas || {};
  const areaChips = ['samlter', 'ilter', 'shimter'].filter(k => areas[k]).map(k => {
    const a = areas[k]; const c = rateColor(a.score);
    return `<div class="v35-area"><span class="v35-area-label">${a.label}</span><span class="v35-area-score" style="color:${c}">${a.score}</span></div>`;
  }).join('');

  const dir = (vision2035.directional || []).map(d => `<span class="v35-dir-chip">${escapeHtml(d.name)} <em>${escapeHtml(d.direction)}</em></span>`).join('');

  const card = document.createElement('section');
  card.id = 'vision-2035-card';
  card.className = 'v35-card';
  card.setAttribute('aria-label', '2035 비전 달성률');
  card.innerHTML = `
    <header class="v35-head">
      <span class="v35-emoji" aria-hidden="true">🎯</span>
      <div class="v35-titles"><h3>2035 비전 달성률</h3><p>2035 남양주 도시기본계획 목표 대비 현황</p></div>
      <span class="v35-badge">도시기본계획</span>
    </header>
    <div class="v35-areas">${areaChips}</div>
    <div class="v35-section-label">수치 지표 달성률 (현황 ÷ 2035 목표)</div>
    <div class="v35-bars">${bars}</div>
    <div class="v35-section-label">방향성 지표 (목표 수치 미명시 — 증감 방향)</div>
    <div class="v35-dirs">${dir}</div>
    ${vision2035.key_insight ? `<div class="v35-insight">💡 ${escapeHtml(vision2035.key_insight)}</div>` : ''}
    <footer class="v35-foot">출처: ${escapeHtml((vision2035._meta && vision2035._meta.source) || '2035 남양주 도시기본계획')} · 현황값은 2020년 통계연보 기준(갱신 필요)</footer>`;

  const anchor = document.getElementById('ai-interpretation-card') || document.getElementById('sigun-insight-card') || document.getElementById('score-cards');
  if (anchor && anchor.parentNode) anchor.parentNode.insertBefore(card, anchor.nextSibling);
  else { const cd = document.getElementById('city-detail'); if (cd) cd.appendChild(card); }
}
...
// ===================================================================
// === 0602: 근거 칩 공용 컴포넌트 (출처/비교/근거 표면화) ===
// ===================================================================
// kind: 'source'(회색·출처) | 'rank'(등급색·비교/순위) | 'basis'(파랑·근거문서)
// rankTier: 'high'|'low'|'mid'|'wait' (rank 칩 색상)
function makeFactChip(kind, label, opts) {
  opts = opts || {};
  const ico = opts.icon != null ? opts.icon
    : (kind === 'source' ? '📍' : kind === 'basis' ? '📄' : '📊');
  const tier = opts.tier ? ` is-${opts.tier}` : '';
  const title = opts.tooltip ? ` title="${escAttr(opts.tooltip)}"` : '';
  const icoHtml = ico ? `<span class="fc-ico" aria-hidden="true">${ico}</span>` : '';
  return `<span class="fact-chip fact-chip--${kind}${tier}"${title}>${icoHtml}${escHtml(label)}</span>`;
}
// 안전한 텍스트/속성 이스케이프 (기존 escapeHtml이 있으면 그걸, 없으면 폴백)
function escHtml(s) {
...
// 0602: 객관적 비교 기준 칩 (#5) — 전국 농촌 평균·유사 지자체 평균.
// 값이 있으면 점등, 없으면(placeholder) "데이터 대기 ⏳" 회색 칩.
function makeBenchmarkChips(key, unit) {
  const u = unit || '';
  const b = benchmarks && benchmarks.indicators && benchmarks.indicators[key];
  const chip = (label, val) => (val != null)
    ? makeFactChip('rank', `${label} ${val}${u}`, { icon: '🌐', tooltip: '외부 통계 비교 기준' })
    : makeFactChip('rank', `${label} 데이터 대기`, { icon: '⏳', tier: 'wait', tooltip: '외부 통계(KOSIS·SGIS) 수집 후 자동 점등' });
  return [chip('전국농촌', b ? b.national_rural_avg : null), chip('유사지자체', b ? b.peer_avg : null)].join('');
}
...
function drawDongCompareChart(dongs) {
  ...
  // 0602: 9개 농촌 읍면 전체 평균선 (객관적 내부 비교 기준 #5)
  const allVals = dongs.map(d => getEupIndicator(d.adm_nm, indicator.key))
    .filter(r => r && r.value != null).map(r => r.value);
  const eupAvg = allVals.length ? Math.round(allVals.reduce((a, b) => a + b, 0) / allVals.length * 10) / 10 : null;
  ...
  if (eupAvg != null && rows.length > 1) {
    datasets.push({
      type: 'line', label: `읍면 평균 ${eupAvg}${indicator.unit || ''}`,
      data: rows.map(() => eupAvg),
      borderColor: '#5B21B6', borderWidth: 1.5, borderDash: [5, 4],
  ...
  dongCompareChart = new Chart(canvas.getContext('2d'), {
    ...
    options: {
      ...
      plugins: {
        legend: { display: eupAvg != null && rows.length > 1, labels: { filter: (it) => it.text.includes('읍면 평균'), font: { size: 11 } } },
        tooltip: {
          callbacks: {
  ...
  if (beforeCanvas) {
    state.charts.scenarioBefore = new Chart(beforeCanvas, {
      ...
      options: {
        ...
        scales: { r: { min: 0, suggestedMax: maxVal, ticks: { font: { size: 11 } }, pointLabels: { font: { size: 12 } } } },
...
function getEupIndicator(eupName, canon) {
  ...
  // 0602: 값이 숫자(레거시) 또는 {value,source,memo,updated_at}(신규) 둘 다 허용
  const ov = state.eupOverrides && state.eupOverrides[eupName];
  if (ov && ov[canon] != null && ov[canon] !== '') {
    const rec = ov[canon];
    const val = (typeof rec === 'object') ? rec.value : rec;
    if (val != null && val !== '') {
      ...
      return {
        value: Number(val), source: 'officer',
        officerSource: (typeof rec === 'object' && rec.source) ? rec.source : '',
        officerMemo: (typeof rec === 'object' && rec.memo) ? rec.memo : '',
...
async function llmInterpret(scope, regionId, context, opts = {}) {
  ...
  recordLlmUsage(data); // 0602 #9: 토큰 사용량 누적
...
/** 현재 화면 맥락 + 포커스 지표 facts 수집 (정책 라이브러리·2035 달성률 포함) */
...
function attachPolicyHints(ctx) {
  const policies = [];
  const refCity = state.selectedCity || 'namyangju';
  const fk = ctx.focusIndicator && ctx.focusIndicator.key;
  if (fk && typeof getIndicatorInsight === 'function') {
    const ins = getIndicatorInsight(refCity, fk);
    if (ins && ins.tierText) policies.push(`[${ctx.focusIndicator.name} 등급별 정책] ${ins.tierText}`);
  }
  // 읍면: 발화 트리거의 민선8기 연계 정책 제안
  if (state.selectedDong && triggerConfig) {
    const eupName = (simulationData && simulationData.dongs && simulationData.dongs[state.selectedDong] && simulationData.dongs[state.selectedDong].adm_nm) || '';
    if (eupName && typeof buildInsightCards === 'function') {
      const all = getEupAllIndicators(eupName);
      const fired = firedTriggerIds(all);
      buildInsightCards(eupName, all, fired).slice(0, 3).forEach(c => {
        if (c.policy) policies.push(`[${c.title}] ${c.policy}`);
      });
    }
  }
  if (policies.length) ctx.policies = policies.slice(0, 4);
}

/** 2035 도시기본계획 비전 달성률을 facts로 첨부 (남양주 한정) */
function attachVision2035Facts(ctx) {
  if (!vision2035 || !ctx.region || !ctx.region.includes('남양주')) return;
  const areas = vision2035.areas || {};
  ctx.vision2035 = {
    areas: Object.keys(areas).map(k => `${areas[k].label} ${areas[k].score}점`),
    low_rate: (vision2035.numeric || []).filter(n => n.rate < 60).map(n => `${n.name} ${n.rate}%(목표 ${n.target}${n.unit})`),
    high_rate: (vision2035.numeric || []).filter(n => n.rate >= 80).map(n => `${n.name} ${n.rate}%`),
    key_insight: vision2035.key_insight,
  };
}

/** 현재 화면 맥락 + 포커스 지표 facts (원본) */
function _buildAskContextRaw() {
  const ctx = {};
...
function renderEupAnalysis(eupName, cityId, info) {
  ...
  renderEupDiagSummary(eupName);   // 0602: 종합 진단 요약 (#6)
...
// ===================================================================
// === 0602: 종합 진단 요약 카드 (#6·#7) — 강점/부족/우선과제/다음할일 ===
// ===================================================================
const _DIAG_AXIS_KO = { T: '교통·접근', H: '삶의 질', E: '교육·생태' };

function _diagSummaryHtml(opts) {
  // opts: {headline, strengths[], weaknesses[], tasks[], nexts[], adminNext:bool}
  const li = (xs, empty) => (xs && xs.length)
    ? `<ul>${xs.slice(0, 4).map(s => `<li>${escHtml(s)}</li>`).join('')}</ul>`
    : `<p class="diag-empty">${empty || '해당 없음'}</p>`;
  const nextCls = opts.adminNext ? ' admin-only' : '';
  return `
    <div class="diag-card">
      <div class="diag-head"><span class="diag-head-ico">🧭</span><h3>종합 진단 요약</h3>
        ${opts.headline ? `<span class="diag-headline">${escHtml(opts.headline)}</span>` : ''}</div>
      <div class="diag-grid">
        <div class="diag-col diag-col--strength"><h4>💪 강점</h4>${li(opts.strengths, '두드러진 강점 없음')}</div>
        <div class="diag-col diag-col--weak"><h4>⚠️ 부족한 점</h4>${li(opts.weaknesses, '큰 취약점 없음')}</div>
        <div class="diag-col diag-col--task"><h4>🎯 우선 개선 과제</h4>${li(opts.tasks, '준비된 과제 없음')}</div>
        <div class="diag-col diag-col--next${nextCls}"><h4>✅ 담당자 다음 할 일</h4>${li(opts.nexts, '다음 단계 데이터 대기')}</div>
      </div>
      ${opts.adminNext ? '<p class="diag-foot">‘담당자 다음 할 일’은 읍면담당자 모드에서 강조됩니다.</p>' : ''}
    </div>`;
}

/** 읍면 종합 진단 — 트리거 카드 + 비전 점수 종합 */
function renderEupDiagSummary(eupName) {
  const host = document.getElementById('eup-diag-summary');
  if (!host) return;
  if (!triggerConfig) { host.innerHTML = ''; return; }
  const data = getEupAllIndicators(eupName);
  const fired = firedTriggerIds(data);
  const cards = buildInsightCards(eupName, data, fired);
  const vs = (typeof visionScore === 'function') ? visionScore(eupName) : null;

  const strengths = [], weaknesses = [], tasks = [], nexts = [];
  cards.forEach(c => {
    if (c.color === 'green' || c.color === 'blue') strengths.push(c.title);
    else if (c.color === 'red' || c.color === 'amber') weaknesses.push(c.title);
    if (c.policy) tasks.push(c.policy.replace(/^[①-⑩\s]*/, '').split(/[②③④⑤]/)[0].trim().slice(0, 80));
    if (c.next) nexts.push(c.next.split(/[.→]/)[0].trim().slice(0, 70));
  });

  // 비전 축 기반 보강
  let headline = '';
  if (vs && vs.overall != null) {
    const axisEntries = Object.entries(vs.axes || {}).filter(([, v]) => v != null);
    if (axisEntries.length) {
      axisEntries.sort((a, b) => a[1] - b[1]);
      const low = axisEntries[0], high = axisEntries[axisEntries.length - 1];
      headline = `비전 적합도 ${vs.overall}점 · 최저 축 ${_DIAG_AXIS_KO[low[0]] || low[0]}(${low[1]})`;
      if (high[1] >= 60) strengths.push(`${_DIAG_AXIS_KO[high[0]] || high[0]} 비전 축 양호 (${high[1]}점)`);
      if (low[1] < 50) weaknesses.push(`${_DIAG_AXIS_KO[low[0]] || low[0]} 비전 축 취약 (${low[1]}점)`);
    }
    if (vs.potentialFelt && vs.potentialFelt.potential != null && vs.potentialFelt.felt != null
        && vs.potentialFelt.potential - vs.potentialFelt.felt >= 15) {
      weaknesses.push(`자원 잠재(${vs.potentialFelt.potential}) 대비 시민 체감(${vs.potentialFelt.felt}) 낮음`);
    }
  }
  if (!nexts.length) nexts.push('현장조사 항목 갱신 → 비전·트리거 재계산');

  host.innerHTML = _diagSummaryHtml({
    headline,
...
/** 시군 종합 진단 — AI 사전해석 + 2035 달성률 + 지표 순위 */
function renderSigunDiagSummary(cityId) {
  const host = document.getElementById('sigun-diag-summary');
  if (!host) return;
  const ai = (typeof getAiInterpretation === 'function') ? getAiInterpretation(cityId) : null;
  const c = ai && ai.city;
  const strengths = (c && c.strengths) ? c.strengths.slice() : [];
  const weaknesses = (c && c.weaknesses) ? c.weaknesses.slice() : [];
  const tasks = [];
  if (c && c.policy_recommendation) {
    c.policy_recommendation.split(/[,，]/).map(s => s.trim()).filter(Boolean).forEach(s => tasks.push(s));
  }

  // 데이터 기반 다음 할 일 — 가장 취약한 지표 2개
  const nexts = [];
  try {
    const facts = Object.keys(INDICATORS).map(k => sigunIndicatorFact(cityId, k)).filter(Boolean);
    facts.sort((a, b) => b.rank - a.rank); // 순위 큰(나쁜) 순
    facts.slice(0, 2).forEach(f => nexts.push(`${f.name} 보강 (${f.scopeLabel} ${f.rank}/${f.total}위)`));
  } catch (_) {}
  // 2035 최저 달성 영역
  if (cityId === 'namyangju' && vision2035) {
    const low = (vision2035.numeric || []).slice().sort((a, b) => a.rate - b.rate)[0];
    if (low) nexts.push(`2035 목표 점검: ${low.name} (현 달성률 ${low.rate}%)`);
  }

  host.innerHTML = _diagSummaryHtml({
    headline: (c && c.headline) ? c.headline : '',
    strengths, weaknesses, tasks, nexts, adminNext: false,
  });
}
...
function renderEupFieldEditPanel(eupName) {
  ...
  // 0602: ov[k] 가 숫자(레거시) 또는 {value,source,memo} 객체 둘 다 허용
  const ovVal = (k) => { const r = ov[k]; if (r == null || r === '') return ''; return (typeof r === 'object') ? (r.value != null ? r.value : '') : r; };
  const ovSrc = (k) => { const r = ov[k]; return (r && typeof r === 'object' && r.source) ? r.source : ''; };
  const ovMemo = (k) => { const r = ov[k]; return (r && typeof r === 'object' && r.memo) ? r.memo : ''; };
  const SRC_OPTS = ['현장조사', '시청·행정자료', '추정·보정', '기타'];
  ...
  const rows = EUP_FIELD_INPUT_KEYS.map(k => {
    ...
    const curTxt = (cur && cur.value != null)
      ...
      : '현재값 없음';
    const srcSel = ovSrc(k);
    const opts = ['<option value="">출처 선택…</option>']
      .concat(SRC_OPTS.map(s => `<option value="${escapeHtml(s)}"${s === srcSel ? ' selected' : ''}>${escapeHtml(s)}</option>`)).join('');
    return `
      <div class="eupedit-row eupedit-row--v2">
        ...
        <input type="number" step="any" class="eupedit-input" data-eup-key="${k}"
               value="${ovVal(k)}" placeholder="${cur && cur.value != null ? cur.value : '입력'}" aria-label="${escapeHtml(name)} 값">
        <span class="eupedit-unit">${escapeHtml(unit)}</span>
        <select class="eupedit-srcsel" data-eup-key="${k}" aria-label="${escapeHtml(name)} 출처">${opts}</select>
        <input type="text" class="eupedit-memo" data-eup-key="${k}" value="${escapeHtml(ovMemo(k))}" placeholder="메모(선택)" aria-label="${escapeHtml(name)} 메모">
      </div>`;
  }).join('');
  ...
  host.innerHTML = `
    ...
    <div class="eupedit-card">
      <p class="eupedit-hint">${escapeHtml(eupName)}의 현장조사값을 <b>출처·메모와 함께</b> 입력하면 비전 적합도·트리거·시사점이 즉시 다시 계산됩니다. 입력 출처는 데이터에 함께 기록됩니다. (이 브라우저에만 저장 — 공유·영구 저장은 별도 백엔드 필요)</p>
  ...
  document.getElementById('eupedit-save').addEventListener('click', () => {
    const store = {};
    const nowYmd = (() => { try { return new Date().toISOString().slice(0, 10); } catch (_) { return ''; } })();
    host.querySelectorAll('.eupedit-input').forEach(inp => {
      const k = inp.dataset.eupKey;
      const v = inp.value.trim();
      if (v === '' || isNaN(Number(v))) return;
      const src = (host.querySelector(`.eupedit-srcsel[data-eup-key="${k}"]`) || {}).value || '';
      const memo = ((host.querySelector(`.eupedit-memo[data-eup-key="${k}"]`) || {}).value || '').trim();
      store[k] = { value: Number(v), source: src, memo: memo, updated_at: nowYmd };
    });
...
function renderEupLlmResult(eupName, d) {
  ...
  card.innerHTML = `
    ...
    ${buildLlmEvidencePanel('eup', eupName)}
...
/**
 * 0602: ② 지표 선정논리 서사 블록 — 가이드 페이지 상단.
 * 삶/일/쉼 "무엇을 보려는가" + 남양주 자율 8지표 선정 근거.
 */
function buildGuideRationale() {
  const pillars = [
    { cls: 'samlter', icon: '🏘️', label: '삶터', q: '여기서 계속 살 수 있는가?', d: '인구 추세·고령화·생활SOC 등 <b>정주 지속가능성</b>을 봅니다.' },
    { cls: 'ilter',   icon: '🌾', label: '일터', q: '먹고살 일이 있는가?', d: '고용·사업체·농업 세대교체 등 <b>농촌 경제 활력</b>을 봅니다.' },
    { cls: 'shimter', icon: '🌲', label: '쉼터', q: '쉬고 누릴 수 있는가?', d: '녹지·수변·체험·공원 등 <b>자연·여가 향유</b>를 봅니다.' },
  ];
  const pillarCards = pillars.map(p => `
    <div class="grat-pillar grat-pillar--${p.cls}">
      <div class="grat-pillar-top"><span>${p.icon}</span><b>${p.label}</b></div>
      <div class="grat-pillar-q">“${p.q}”</div>
      <p class="grat-pillar-d">${p.d}</p>
    </div>`).join('');
  ...
  try {
    ...
    jayulRows = keys.map(k => {
      const def = (typeof JAYUL_INDICATORS_POOL !== 'undefined' && JAYUL_INDICATORS_POOL[k]) || {};
      const cat = def.category === 'samlter' ? '삶터' : def.category === 'ilter' ? '일터' : def.category === 'shimter' ? '쉼터' : '';
      return `<tr><td class="grat-tbl-key">${escHtml(k)}</td><td>${escHtml(def.name || k)}</td><td>${escHtml(cat)}</td></tr>`;
    }).join('');
  } catch (_) {}

  return `
    <section class="guide-rationale">
      ...
      <div class="grat-jayul">
        ...
        <p class="src-note">남양주는 <b>도농전환형(도심근교)</b> 권역으로 도시 확장·귀촌 압력이 큽니다. 그 특성에 맞춰
          <b>귀촌 정착·농업 전환·체험 휴양</b>을 보는 지표를 자율 선정했습니다.</p>
        <table class="grat-tbl"><thead><tr><th>키</th><th>지표</th><th>분류</th></tr></thead><tbody>${jayulRows}</tbody></table>
      </div>` : ''}
    </section>`;
}
...
// ===================================================================
// === 0602: 발표 목차 네비게이션 (웹페이지 자체가 발표 자료) ===
// ===================================================================
...
function enterPresentMode(step) {
  document.body.classList.add('present-mode');
  goPresentationStep(step || 1);
}
...
function presentStepDiagnosis() {
  hideAllOverlayScreens();
  document.getElementById('landing-screen')?.classList.add('is-hidden');
  if (typeof selectCity === 'function') { try { selectCity('namyangju'); } catch (_) {} }
  // 조안면 자동 드릴다운 (현장조사 데이터 보유 읍면)
  if (typeof selectDong === 'function') { try { selectDong('31130350', '조안면', 'namyangju'); } catch (_) {} }
  setTimeout(() => {
    const t = document.getElementById('eup-diag-summary') || document.getElementById('dong-detail');
    if (t && t.scrollIntoView) t.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, 220);
}
function presentStepSummary() {
  hideAllOverlayScreens();
  document.getElementById('landing-screen')?.classList.add('is-hidden');
  if (typeof clearDongSelection === 'function') { try { clearDongSelection(); } catch (_) {} }
  if (typeof selectCity === 'function') { try { selectCity('namyangju'); } catch (_) {} }
  setTimeout(() => {
    const t = document.getElementById('sigun-diag-summary') || document.getElementById('city-detail');
    if (t && t.scrollIntoView) t.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, 220);
}
function initPresentMode() {
  const spine = document.getElementById('present-spine');
  if (spine) {
    spine.querySelectorAll('.ps-step').forEach(b => {
      b.addEventListener('click', () => goPresentationStep(Number(b.dataset.step)));
    });
    document.getElementById('ps-prev')?.addEventListener('click', () => goPresentationStep(presentStep - 1));
    document.getElementById('ps-next')?.addEventListener('click', () => goPresentationStep(presentStep + 1));
...
// ===================================================================
// === 0602: ③ 데이터·방법론 페이지 (출처표 + 가상데이터 생성방식) ===
// ===================================================================
// 가상 현장조사 4시트 → 산출지표 → 산식 (DATA-SOURCES.md §12-1 기준)
const FIELD_SURVEY_METHOD = [
  { sheet: 'raw_농가조사', unit: '농가 1곳 = 1행', n: '225행',
    cols: '청년경영주_여부(Y/N) · 친환경_인증_여부(Y/N) · 주작목 · 경영주연령대',
    out: 'W5 농업세대교체 · W7 친환경인증농가',
    formula: "W5 = 청년경영주 'Y' 비율(%) · W7 = 친환경인증 'Y' 비율(%)" },
  { sheet: 'raw_귀촌정착', unit: '귀촌가구 1가구 = 1행', n: '354행',
    cols: '전입연도 · 가구원수 · 20_39세_가구원수 · 2026년_주소유지_여부(Y/N)',
    out: 'L6 귀촌3년정착률 · W6 청년귀농유입',
    formula: "L6 = (전입≤2023 & 주소유지 'Y') 비율(%) · W6 = Σ청년가구원/Σ가구원 ×100" },
  { sheet: 'raw_체험텃밭', unit: '시설 1곳 = 1행', n: '45행',
    cols: '시설유형 · 수용가능인원 · 연간프로그램_횟수 · 분양/운영구획',
    out: 'W9 농촌체험 · R6 도시텃밭 · R7 주말농원',
    formula: 'W9 = Σ프로그램/인구 ×1000 · R6 = Σ수용인원/인구 ×100 · R7 = Σ운영/Σ분양 ×100' },
  { sheet: 'SOC_체크', unit: '읍면 1곳 = 1행', n: '9행',
    cols: '12개 생활SOC 항목 0/1 (의료·돌봄·교육·문화·체육·교통·디지털·행정 등)',
    out: 'L4 생활SOC충족지수',
    formula: 'L4 = 보유항목수 / 12 ×100' },
];

function showSourcesPage() {
  renderSourcesPage();
  showOverlayScreen('sources-screen', '#sources');
}

function renderSourcesPage() {
  const host = document.getElementById('sources-body');
  if (!host) return;
  const inds = (indicatorReference && (indicatorReference.indicators || indicatorReference)) || {};
  const order = ['L1','L2','L3','L4','L5','L6','L7','L8','W1','W2','W3','W4','W5','W6','W7','W8','R1','R2','R3','R4','R5','R6','R7','R8'];
  const catClass = (lab) => lab && lab.includes('삶') ? 'samlter' : lab && lab.includes('일') ? 'ilter' : lab && lab.includes('쉼') ? 'shimter' : '';

  // --- A. 지표 출처표 ---
  const rows = order.filter(k => inds[k]).map(k => {
    const m = inds[k];
    const cat = catClass(m.category_label);
    const sec = m.secondary_source && m.secondary_source !== '—' ? m.secondary_source : '';
    const typeChip = m.type === '공공'
      ? makeFactChip('source', '공공데이터', { icon: '🏛️' })
      : makeFactChip('rank', m.type || '기타', { icon: '✍️', tier: 'wait' });
    return `<tr>
      <td class="src-tbl-key">${escHtml(k)}</td>
      <td>${escHtml(m.name || '')}</td>
      <td><span class="src-cat src-cat--${cat}">${escHtml((m.category_label||'').replace('터',''))}</span></td>
      <td class="src-tbl-formula">${escHtml(m.formula || '')}</td>
      <td>${m.url ? `<a href="${escAttr(m.url)}" target="_blank" rel="noopener">${escHtml(m.primary_source||'')}</a>` : escHtml(m.primary_source||'')}</td>
      <td class="src-tbl-sec">${escHtml(sec || '—')}</td>
      <td>${typeChip}</td>
    </tr>`;
  }).join('');
  ...
  const tableHtml = `
    <div class="src-card">
      <h3 class="src-h3">📑 지표별 데이터 출처표 <span class="src-h3-sub">${order.filter(k=>inds[k]).length}개 지표 · 산식 · 1·2차 출처</span></h3>
      <p class="src-note">공공데이터(KOSIS·SGIS)는 대부분 시군 단위까지 제공됩니다. 읍면 단위는 현장조사로 보완합니다.</p>
      <div class="src-tbl-wrap">
        <table class="src-tbl">
          <thead><tr><th>키</th><th>지표</th><th>분류</th><th>산식</th><th>1차 출처</th><th>2차 출처</th><th>유형</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>
    </div>`;
  ...
  // --- B. 가상 현장조사 방법론 카드 ---
  const methodCards = FIELD_SURVEY_METHOD.map(s => `
    <div class="src-method-card">
      <div class="src-method-head"><code>${escHtml(s.sheet)}</code><span class="src-method-n">${escHtml(s.unit)} · ${escHtml(s.n)}</span></div>
      <div class="src-method-cols"><b>입력 컬럼</b> ${escHtml(s.cols)}</div>
      <div class="src-method-out">${escHtml(s.out)}</div>
      <div class="src-method-formula">${escHtml(s.formula)}</div>
    </div>`).join('');

  const methodHtml = `
    <div class="src-card src-card--method">
      <h3 class="src-h3">🧪 가상 현장조사 데이터 — 생성 방식</h3>
      <div class="src-warn">
        ⚠️ <b>9개 농촌 읍면(조안면 등)의 현장조사 값은 ‘가상 데이터’입니다.</b>
        공공데이터가 읍면 단위까지 없어, 실제 조사표 설계 기준으로 가상 응답을 만들어
        <b>플랫폼 구조와 집계 로직을 먼저 검증</b>했습니다. 실제 데이터는
        <span class="fact-chip fact-chip--source">📍 현장조사</span> 배지로 구분되며,
        같은 자리에 실측값을 넣으면 그대로 대체됩니다.
      </div>
      <div class="src-method-grid">${methodCards}</div>
      <p class="src-note">인구 분모는 <code>region-meta.json</code>의 읍면 인구(<code>dong.raw.tot_ppltn</code>)를 사용합니다.
        R5(수변·생태쉼터 면적)는 GIS 산출이 필요해 미수집(<code>_pending</code>) 상태이며 수집 시 자동 점등됩니다.</p>
    </div>`;

  // --- C. 데이터 파이프라인 도식 ---
  const pipeHtml = `
    <div class="src-card">
      <h3 class="src-h3">🔗 데이터 파이프라인</h3>
      <div class="src-pipe">
        <div class="src-pipe-step"><div class="src-pipe-ico">🏛️</div><b>수집</b><span>KOSIS·SGIS API<br/>지방재정365·문화재청<br/>현장조사(읍면)</span></div>
        <div class="src-pipe-arrow">→</div>
        <div class="src-pipe-step"><div class="src-pipe-ico">🗃️</div><b>가공 (3-layer)</b><span>raw(원본+출처)<br/>computed(산식 계산)<br/>manual(직접 입력)</span></div>
        <div class="src-pipe-arrow">→</div>
        <div class="src-pipe-step"><div class="src-pipe-ico">📊</div><b>표출</b><span>지도·비교·트리거<br/>비전 적합도·AI 해석<br/>출처 배지 동반</span></div>
      </div>
    </div>`;

  // --- D. AI API 연결 방식·토큰 사용량 (#9 검토) ---
  const used = llmUsage.calls > 0;
  const apiHtml = `
    <div class="src-card">
      <h3 class="src-h3">🤖 AI 연결 방식 · 토큰 사용량 검토</h3>
      <p class="src-note">실시간 AI 해석·챗은 <b>MindLogic UOS Gateway</b>(학교 게이트웨이)를 <b>Cloudflare Worker 프록시</b> 경유로 호출합니다.
        비밀키는 Worker에만 보관되어 브라우저에 노출되지 않습니다. 비용 절감을 위해 최저가 모델
        <code>gpt-5.4-nano</code>을 사용하고, 순위·평균 등 <b>사실은 JS가 계산</b>해 LLM은 서술만 합니다.</p>
      <div class="src-usage">
        <div class="src-usage-item"><b>${llmUsage.calls}</b><span>이번 세션 호출 수</span></div>
        <div class="src-usage-item"><b>${used ? llmUsage.in.toLocaleString() : '—'}</b><span>입력 토큰(누적)</span></div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    updateComparisonCityList();
    initPresentMode();  // 0602: 발표 목차 네비게이션
  } catch (err) {
